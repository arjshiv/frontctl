...
const ACTION_ROUTE_KIND: Record<string, string> = {
  ...
  "draft.update": "message-or-draft",
  "draft.forward": "message-or-draft",
...
const ACTION_CAPTURE_GUIDES: Record<string, Omit<WriteCaptureGuide, "verified" | "expectedRouteKind" | "fixturePath" | "captureCommand" | "verifyCommand">> = {
  ...
  "draft.update": {
    action: "draft.update",
    safeFrontAction: "Update one existing saved draft in Front without sending it.",
    previewCommand: "frontctl draft update CONVERSATION_ID MESSAGE_UID --to test@example.com --subject \"frontctl draft test\" --body \"Updated draft only\" --json",
    captureName: "draft.update",
    notes: [
      "Create a harmless test draft first, then update only that draft.",
      "Do not click send.",
      "Use the conversation id and message uid returned by frontctl draft compose.",
    ],
  },
  "draft.forward": {
    action: "draft.forward",
    safeFrontAction: "Create one forwarded-message draft to a harmless recipient without sending it.",
    previewCommand: "frontctl draft forward CONVERSATION_ID --to test@example.com --body \"Forward note\" --json",
    captureName: "draft.forward",
    notes: [
      "Do not click send.",
      "Use a harmless recipient such as test@example.com and discard the saved draft after verification.",
      "Capture draft save/update separately from discard.",
    ],
  },
  "draft.discard": {
...
export const WRITE_ACTION_SPECS = [
  ...
  },
  {
    ...
    body: {
      author_id: 456,
      from: { channel_id: 789 },
      subject: "Fw: frontctl draft test",
      recipients: [{ role: "to", handle: "test@example.com", name: "Test", source: "email" }],
      attachments: [],
      html: "<div>draft-placeholder</div>",
      text: "draft-placeholder",
      ...
      quote_include: false,
      quote_modified: false,
      forward_html: "<div>forward-placeholder</div>",
